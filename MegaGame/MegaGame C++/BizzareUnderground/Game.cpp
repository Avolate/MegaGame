...
gameOver(false), isPaused(false), levelComplete(false)
{
    ...
    sf::Image icon;
    // Ïûòàåìñÿ çàãðóçèòü èêîíêó èç ðàçíûõ ïóòåé
    if (icon.loadFromFile("D:\\MegaGame\\MegaGame\\MegaGame C++\\BizzareUnderground\\assets\\icon.png"))
    {
        window.setIcon(icon.getSize().x, icon.getSize().y, icon.getPixelsPtr());
    }

    // Çàãðóæàåì øðèôò äëÿ òåêñòà
    if (!font.loadFromFile("arial.ttf"))
    {
        font.loadFromFile("C:\\Windows\\Fonts\\arial.ttf");
    }

    // ========== ÇÀÃÐÓÆÀÅÌ ÔÎÍÎÂÓÞ ÊÀÐÒÈÍÊÓ ==========
    if (!backgroundTexture.loadFromFile("D:\\MegaGame\\MegaGame\\MegaGame C++\\BizzareUnderground\\assets\\background.png") &&
    ...
    // ========== ÇÀÃÐÓÆÀÅÌ ÒÅÊÑÒÓÐÓ GROUND ÏÅÐÅÄ ÈÍÈÖÈÀËÈÇÀÖÈÅÉ ÓÐÎÂÍß ==========
